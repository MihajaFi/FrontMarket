...
export default function AdminDashboard() {
  ...
  const [salesByMerchant, setSalesByMerchant] = useState<{ name: string; totalSales: number }[]>([]);
  const [stockByCategory, setStockByCategory] = useState<{ category: string; stock: number }[]>([]);
  const [salesByMonth, setSalesByMonth] = useState<{ month: string; ventes: number; commandes: number }[]>([]);
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        const [dashboardData, merchantsData, salesMonthData, ordersData, stockData] = await Promise.all([
          dashboardService.getDashboard(),
          dashboardService.getTotalSaleByMerchant(),
          dashboardService.getSalesByMonth(),
          orderService.getAll(),
          merchantProductService.getStockByCategory(),
        ]);

        setDashboard(dashboardData);
        setSalesByMerchant(merchantsData);
  ...
  if (loading || !dashboard) {
    return <Layout title="Tableau de bord">Chargement...</Layout>;
  }
  ...
  const statCards = [
    ...
    {
      label: 'Commerçants',
      value: dashboard.totalMerchant,
      icon: Store,
      iconBg: 'hsl(var(--stat-merchants) / 0.12)',
      iconColor: 'hsl(var(--stat-merchants))',
      change: `${dashboard.totalMerchant} total`,
      positive: true,
    },
    {
      label: 'Commandes en attente',
      value: dashboard.orderPending,
      icon: ShoppingCart,
      iconBg: 'hsl(var(--stat-stock) / 0.12)',
      iconColor: 'hsl(var(--stat-stock))',
      change: `${list.filter(o => o.status === 'PENDING').length} total`,
      positive: null,
    },
  ];
          ...
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={salesByMerchant} cx="50%" cy="50%" innerRadius={55} outerRadius={90}
                dataKey="totalSales" paddingAngle={3}>
          ...
          <tbody>
            {filtered.map(order => (
              <tr key={order.id}>
                ...
                <td style={{ fontWeight: 600 }}>{order.totalAmount.toLocaleString()} Ar</td>
                <td style={{ color: 'hsl(var(--muted-foreground))' }}>{order.orderDate}</td>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    PAID: { label: 'Validée', cls: 'badge-validated' },
