      ...
      <div className="container py-6">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-3">
            {items.map(({ product, quantity, promotionValue }) => {
              const finalPrice = product.price
                ? Math.max(product.price - (promotionValue || 0), 0)
                : undefined;
                ...
                <div key={product.id} className="bg-card border border-border rounded-lg p-4 flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      {promotionValue && product.price && (
                        <span className="text-sm line-through text-muted-foreground">
                          {formatPrice(product.price)}
                        </span>
                      )}
                      <span className="font-display font-bold text-primary">
                        {finalPrice ? formatPrice(finalPrice) : "Prix sur demande"}
                      </span>
                    </div>
